...
//        convertView = getConvertView(node, position, convertView, parent);
        // 设置内边距
        holder.itemView.setPadding(node.getLevel() * 60, 3, 3, 3);
        /**
    ...
    protected void setNodeParentChecked(Node node, boolean checked){
     /*   if(checked){
           // node.setChecked(checked);
            if(node.getParent()!=null)
                setNodeParentChecked(node.getParent(), checked);
    ...
    protected void setNodeParentChecked2(Node node, boolean checked){
     /*   if(checked){
           // node.setChecked(checked);
            if(node.getParent()!=null)
                setNodeParentChecked(node.getParent(), checked);
